...
import { Trash2, CheckCircle, Circle, Calendar, Flag, Edit2, X, Filter, PlusCircle } from 'lucide-react';
...
const TaskList = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isEditing) { await updateTask(editId, newTask); setIsEditing(false); setEditId(null); } 
  ...
  const startEdit = (task) => {
    ...
    setIsEditing(true); setEditId(task.id); window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const cancelEdit = () => { setIsEditing(false); setEditId(null); setNewTask({ title: '', description: '', status: 'Pending', priority: 'Medium', dueDate: '' }); };
  const handleStatusToggle = (task) => { const newStatus = task.status === 'Completed' ? 'Pending' : 'Completed'; updateTask(task.id, { status: newStatus }); };
  const getPriorityColor = (p) => { if (p === 'High') return 'text-red-700 bg-red-50 border-red-100'; if (p === 'Medium') return 'text-yellow-700 bg-yellow-50 border-yellow-100'; return 'text-blue-700 bg-blue-50 border-blue-100'; };
    ...
    <div>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {filteredTasks.map((task) => (
          <div key={task.id} className={`group relative p-5 rounded-2xl bg-white shadow-sm border border-slate-100 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${task.status === 'Completed' ? 'border-l-8 border-l-green-500 bg-green-50/30' : 'border-l-8 border-l-amber-500 hover:border-l-blue-500'}`}>
            <div className="flex justify-between items-start">
              <div><h4 className={`text-lg font-bold ${task.status === 'Completed' ? 'text-slate-500 line-through' : 'text-slate-800'}`}>{task.title}</h4><p className="mb-3 text-slate-500 text-sm">{task.description}</p></div>
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button onClick={() => deleteTask(task.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-all"><Trash2 size={18}/></button>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-2">
              <span className={`px-3 py-1 text-xs rounded-full font-medium ${task.status === 'Completed' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'}`}>{task.status}</span>
              <button onClick={() => handleStatusToggle(task)} className="text-slate-300 hover:text-blue-600 transition-all transform hover:scale-110">
                {task.status === 'Completed' ? <CheckCircle className="text-green-500 drop-shadow-sm" size={28}/> : <Circle size={28}/>}
              </button>
            </div>
          </div>
        ))}
